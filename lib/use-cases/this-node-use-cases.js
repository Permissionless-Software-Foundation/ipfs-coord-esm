...
class ThisNodeUseCases {
  ...
  async enforceWhitelist () {
    try {
      ...
      console.log(`Reviewing ${allPeers.length} peers...`)
      for (let i = 0; i < allPeers.length; i++) {
        ...
        if (!foundPeer.length) {
          ...
          // Note: do not await. Just issue the command and move on.
          // this.adapters.ipfs.disconnectFromPeer(thisPeer.peer.toString())

          // console.log('this.adapters.ipfs.ipfs.swarm: ', this.adapters.ipfs.ipfs.swarm)
          // const cmd = `ipfs swarm limit peer:${thisPeer.peer.toString()} /home/trout/tmp/ipfs/limit.json`
          // const cmd = `ipfs swarm limit peer:${thisPeer.peer.toString()}`
          // console.log('executing cmd: ', cmd)
          // shell.exec(cmd)
        } else {
          // Add the peer to the Allow List for the Resource Manager
          // https://github.com/ipfs/kubo/blob/master/docs/config.md#swarmresourcemgrallowlist

          // Get the current allow list
          const allowList = await this.adapters.ipfs.ipfs.config.get('Swarm.ResourceMgr.Allowlist')
          console.log('Current allowList: ', allowList)

          // See if the current peer is in the allow list
          const peerIsInAllowList = allowList.filter(x => x === thisPeer.peer.toString())

          if (!peerIsInAllowList.length) {
            allowList.push(thisPeer.peer.toString())

            await this.adapters.ipfs.ipfs.config.set('Swarm.ResourceMgr.Allowlist', allowList)

            console.log(`Peer ${thisPeer.peer} added to Allowlist`)
            console.log('allowList: ', allowList)
          }
        }
      }
      console.log(`----->...Finished reviewing ${allPeers.length} peers.<-----`)
      ...
      console.error('Error in enforceWhitelist(): ', err)
      throw err
    }
  }
}
